...
module.exports = {
  ...
  postTransformNode (ast, options, flags) {
    if (!ast.parent && options[HASFLAG]) {
      if (ast[flagMeta] && !evaluateFlagDir(ast, options, flags)) {
        Object.keys(ast).forEach(k => delete ast[k])
        Object.assign(ast, {
          type: TXT_NODE, text: 'removed by ' + PLUGIN_NAME, isComment: true
        })
  ...
  preTransformNode (ast, options) {
    ...
    if (!ast.parent) {
      onWarn(
        `You are using ${dir} on component root element: ${serializeNode(ast)}, ` +
        'please consider using "files" option.'
      )
    }
    ...
    assert.ok(
      prevNode.type === 1 && prevNode[flagMeta],
      `Unexpected error in ${PLUGIN_NAME} at: ${serializeNode(prevNode)}`
    )
    ast[flagMeta] = {
