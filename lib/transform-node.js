...
function transformNode (ast, flags, useComment) {
  ...
  for (let i = 0; i < ast.children.length; i++) {
    ...
    try {
      ...
      throw genFlagDirError(child, `Unknown flag name: ${JSON.stringify(value)}, ${e.message}`)
    }
    if (keep) {
      transformNode(child, flags, useComment)
    } else {
...
function preTransformNode (ast) {
  ...
  if (checkDir && checkDir.length) {
    ...
    if (checkDir.length > 1) {
      throw genFlagDirError(ast, `"${checkDir.map(v => v[0])}" can not be used together`)
    }
    const [dir, value] = checkDir[0]
    if (dir === IF_FLAG || dir === ELIF_FLAG) {
      if (value === '') throw genFlagDirError(ast, `${dir} can not be empty`)
    } else {
      if (value !== '') throw genFlagDirError(ast, `${dir} must be empty`)
    }
