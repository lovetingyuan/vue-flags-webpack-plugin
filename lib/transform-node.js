...
function onError (options, msg, node) {
  ...
  if (options.warn.length === 2) {
    options.warn(message, false) // false means error type not warning
  } else {
    // add in 2.6.0, node is range actually, should set `outputSourceRange` to true
    options.warn(message, node, false)
  }
...
function preTransformNode (ast, options, { flags }) {
  ...
  if (conditionDirs.some(dir => hasAttribute(ast, dir))) {
    return onError(options, `v-*-flag can not be used with ${conditionDirs}`, ast)
  }
  ...
  if (dir === IF_FLAG) {
    ...
    if ((ast.parent && ast.parent.scopedSlots) || slotDirs.some(dir => hasAttribute(ast, dir))) {
