 ...
 * 如果当前节点有
 * 1  [[1]] // 进入为1， 推入新的组，成为进推
 * 2  [[1, 2]] // 迭代取出最后一个组，推入值，称为常规迭代
 * 3  [[1, 2, 3]] // 常规迭代
 *  1 [[1, 2, 3], [1]] // 进推
 *  2 [[1, 2, 3], [1, 2]] // 常规迭代
 *  1 [[1, 2, 3], [1]] // 清空最后一组，推入值，称为清空迭代
 *  2 [[1, 2, 3], [1, 2]] // 常规迭代
 *    1 [[1, 2, 3], [1, 2], [1]] // 进推
 *    2 [[1, 2, 3], [1, 2], [1, 2]] // 常规迭代
 *  1 [[1, 2, 3], [1]] // 退出为1，推出最后一组，清空新的末尾组，推入值
 *    1 [[1, 2, 3], [1], [1]] // 进推
 *    2 [[1, 2, 3], [1], [1, 2]] // 常规迭代
 *    3 [[1, 2, 3], [1], [1, 2, 3]] // 常规迭代
 *  3 [[1, 2, 3], [1, 3]] // 推出最后一组，
 * 1 [[1]]
 * 2 [[1, 2]]
 * 1
...
function nodePositionType (a, b) {
  // -1: a is parent of b, 0: a and b is brother, 1: a is son of b
  if (!a) {
    return -1
  }
  ...
  let node = b.parent
  while (node && node.parent !== a.parent) {
    node = node.parent
  }
  if (node && node.parent === a.parent) {
    return -1
  }
  return 1
}

let flagDirExpsList = null
let prevFlagNode = null
// const blankNodes = []
module.exports = {
  preTransformNode,
  postTransformNode,
  staticKeys: [flagMeta],
  _preTransformNode (ast, options, flags) {
    if (!ast.parent) {
      flagDirExpsList = []
      prevFlagNode = null
    }
    ...
    ast[flagMeta] = { name: dir }
    let exps
    const positionType = nodePositionType(prevFlagNode, ast)
    if (positionType === -1) {
      exps = [value]
      flagDirExpsList.push(exps)
      if (dir !== IF_FLAG) { throw new Error('unexpected error') }
    } else {
      if (positionType === 1) {
        flagDirExpsList.pop()
      }
      exps = flagDirExpsList[flagDirExpsList.length - 1]
      if (dir === IF_FLAG) {
        exps.length = 0
      }
      exps.push(value)
    ...
    // const keep = toFunc(expression).call(flags)
    ast.expression = expression;
    // if (!keep) {
      // return {
      //   type: TXT_NODE,
      //   text: `<${ast.tag} ${dir}=${JSON.stringify(value)}> was removed.`,
      //   isComment: true
      // }
    // }
  }
