...
function serializeNode (node) {
  ...
  if (node.type === ELE_NODE) {
    const attrs = Object.keys(node.attrsMap).map(name => {
      let attr = name
      let value = node.attrsMap[name]
      if (node[flagMeta] && name === 'data-' + node[flagMeta].name) {
        attr = node[flagMeta].name
        value = node[flagMeta].value
      }
...
function preTransformNode (ast, options, dev) {
  ...
  if (dev) {
    addAttribute(ast, 'data-' + dir, value)
  }
  if (dir === IF_FLAG) {
    ast[flagMeta] = {
