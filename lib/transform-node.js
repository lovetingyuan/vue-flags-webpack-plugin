...
function addAttribute (node, name, value) {
  if (!node.attrsMap[name]) {
    node.attrsMap[name] = value
    node.attrsList.push({ name, value })
  }
  if (node.rawAttrsMap && !node.rawAttrsMap[name]) {
    node.rawAttrsMap[name] = { name, value }
  }
}
...
function serializeNode (node) {
  ...
  if (node.type === ELE_NODE) {
    const attrs = Object.keys(node.attrsMap).map(k => `${k}=${JSON.stringify(node.attrsMap[k])}`)
...
function processElement (node, options, flags) {
  ...
  if (node.ifConditions && !node.ifConditions[IFPROCESSED]) {
    ...
    node.ifConditions.forEach(condition => {
      processElement(condition.block, options, flags)
    })
  }
  if (node.scopedSlots) {
...
function preTransformNode (ast, options, dev) {
  ...
  // not allow to use v-*-flag with v-if/v-else-if/v-else
  if (conditionDirs.some(dir => hasAttribute(ast, dir))) {
    return onError(options, `${dir} can not be used with ${conditionDirs}`, ast)
  }
  removeAttribute(ast, dir)
  if (dev) {
    addAttribute(ast, `data${dir.slice(1)}`, value)
  }
  if (dir === IF_FLAG) {
    ast[flagMeta] = {
  ...
  // allow to use v-if-flag but not v-else-flag or v-elif-flag on scope slot node
  if (ast.parent.scopedSlots || scopedSlotDirs.some(dir => hasAttribute(ast, dir))) {
