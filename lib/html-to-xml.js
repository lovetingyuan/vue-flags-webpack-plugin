/**
 * code copied from:
 * https://github.com/vuejs/vue/blob/master/src/compiler/parser/html-parser.js
 * only removed es6 module syntax
 */
...
module.exports = function toXML (html, loaderContext) {
  const results = []
  ...
  parseHTML(html, {
    ...
    warn (msg) {
      ...
      const err = new Error(`[Vue template parser]: ${msg}`)
    ...
    start (tag, attrs, unary) {
      ...
      for (let { name, value } of attrs) {
        results.push(` ${name}="${(value || '').replace(/"/g, '&#34;')}"`)
      }
      results.push((unary ? '/' : '') + '>')
    },
    end (tag) {
