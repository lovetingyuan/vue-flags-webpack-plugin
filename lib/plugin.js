...
module.exports = class VueFlagsWebpackPlugin {
  ...
  walkRules (rules, cb) {
    for (let rule of rules) {
      // do not support use function as use
  ...
  patchRules (rawRules, dev, vueLoaderPath) {
    ...
    if (vueLoaderPath) {
      ...
      this.walkRules(rules, (rule, index) => {
        if (rule.resource && !(rule.resource('foo.vue') || rule.resource('foo.vue.html'))) {
          return
        }
        const uses = rule.use
        if (typeof uses[index] !== 'object') return // not support use function as use
