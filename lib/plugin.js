...
module.exports = class VueFlagsWebpackPlugin {
  ...
  findVueRule (rules, vueLoaderPath) {
    ...
    rules.forEach(rule => {
      ...
      if (!(rule.resource('foo.vue') || rule.resource('foo.vue.html'))) return
      ...
      if (vueUseIndex !== -1) {
        vueMatches.push([rule.use, vueUseIndex])
      }
      // vue-loader does not support "oneOf" and "rules"
      if (rule.oneOf) {
        vueMatches.push(...this.findVueRule(rule.oneOf, vueLoaderPath))
      }
      if (rule.rules) {
        vueMatches.push(...this.findVueRule(rule.rules, vueLoaderPath))
      }
    })
    return vueMatches
  }
