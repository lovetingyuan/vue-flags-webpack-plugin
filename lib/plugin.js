...
module.exports = class VueFlagsWebpackPlugin {
  ...
  _patchRules () {
    const { rules } = new RuleSet(this.webpackOptions.module.rules)
    const dev = this.webpackOptions.mode === 'development'
    try {
        ...
        vueMatches.forEach(({ uses, index }) => {
          ...
          vueLoader.options.compilerOptions.modules.unshift({
            postTransformNode (ast, options) {
              transformFlags(ast, options, self.pluginOptions, dev)
            }
          })
    ...
    this.webpackOptions.module.rules = rules
  }
  apply (compiler) {
    if (!compiler.hooks) {
    ...
    if (this.webpackOptions.plugins.filter(p => p.constructor === VueFlagsWebpackPlugin).length > 1) {
