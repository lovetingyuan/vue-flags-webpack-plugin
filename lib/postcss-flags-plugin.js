...
const { POSTCSS_SUPPORT_PROPERTY: PSR, RESOLVED_FLAGS_PATH, PLUGIN_NAME } = require('./constants')
...
module.exports = function postcssFlagsPlugin (flags) {
  return function supportsFlagsPlugin (css, ret) {
    ...
    css.walkAtRules(function (rule) {
      ...
      const flagsProps = properties.filter(v => v === PSR)
      if (!flagsProps.length) return
      if (flagsProps.length !== properties.length) {
        ret.warn(`${pluginName}: "${PSR}" in "@supports" can not be used with other properties at\n${chalk.yellow(rule)}`)
        return
      }
      if (missingValue) {
        ret.warn(`${pluginName}: value of "${PSR}" is missing at\n${chalk.yellow(rule)}`)
        return
      }
