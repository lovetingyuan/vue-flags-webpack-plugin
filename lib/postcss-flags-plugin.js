...
module.exports = function postcssFlagsPlugin (flags) {
  return function supportsFlagsPlugin (css, ret) {
    ...
    if (!pluginOptions) {
      ...
      if (isPlainObject(flags)) {
        ret.warn(`${pluginName}: will use flags resolved from ${PLUGIN_NAME}, flags received as parameter will be ignored!`)
      }
    ...
    function onAtRule (rule) {
      ...
      if (watch) {
        ...
        if (!ret.messages.find(({ file }) => file === RESOLVED_FLAGS_PATH)) {
          ret.messages.push({
