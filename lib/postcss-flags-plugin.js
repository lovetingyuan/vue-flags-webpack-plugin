...
module.exports = function postcssFlagsPlugin (flags) {
  ...
  return function supportsFlagsPlugin (css, ret) {
    css.walkAtRules(function (rule) {
      ...
      if (notHandle) {
        if (hasFlag) {
          ret.warn(`postcss-flags-plugin: "${POSTCSS_SUPPORT_PROPERTY}" in "@supports" can not be used with other properties at\n${chalk.yellow(rule)}`)
        }
        return
      }
      ...
      try {
        ...
        throw new Error(`\npostcss-flags-plugin: Unknown flag name(${e.message}) at\n${chalk.red(rule)}\n`)
      }
    })
  }
}
