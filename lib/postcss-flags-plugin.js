...
function postcssFlagsPlugin () {
  return function supportsFlagsPlugin (css, ret) {
    function onAtRule (rule) {
      ...
      if (options.cachedFlagsPath) {
        ret.messages = ret.messages || []
        if (!ret.messages.find(({ file }) => file === options.cachedFlagsPath)) {
          ret.messages.push({
            type: 'dependency', // only works with postcss-loader
            file: options.cachedFlagsPath
          })
        }
      }
    }
