...
function loadFlags (flagsFilePath, isDir) {
  ...
  if (equalObj(pluginOptions.flags || {}, newFlags)) { return }
...
function setOptions (options, compiler) {
  ...
  if (files && Object.keys(files).length) {
    const filterObj = files
    pluginOptions.allFiles = resolveFiles(filterObj, true)
    const filterList = []
    if (!watch) {
      filterList.push(...resolveFiles(filterObj))
    }
    if (!pluginOptions.files) {
      Object.defineProperty(pluginOptions, 'files', {
        get () {
          return watch ? resolveFiles(filterObj) : filterList
        }
      })
    }
  }
...
function resolveFiles (files, all) {
  const filters = []
  Object.keys(files).forEach(flagName => {
    if (!all && pluginOptions.flags[flagName]) { return }
