...
function getFilters (filterMap) {
  const filters = []
  filterMap.forEach((regs, key) => {
    try {
      if (!(key.call(pluginOptions.flags))) {
        filters.push(...regs)
      }
    } catch (e) {
...
function setOptions (options, compiler) {
  ...
  if (files && Object.keys(files).length) {
    ...
    const filterList = watch ? [] : getFilters(filterMap)
    if (!pluginOptions.files) {
      Object.defineProperty(pluginOptions, 'files', {
        enumerable: true,
        get () {
          return watch ? getFilters(filterMap) : filterList
        }
      })
    }
  }
