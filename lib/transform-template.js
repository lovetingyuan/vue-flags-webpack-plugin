const chalk = require('chalk')
const domserializer = require('dom-serializer')

const { IF_FLAG, ELIF_FLAG, ELSE_FLAG, RESOLVED_FLAGS_PATH } = require('./constants')
const testDirective = new RegExp(`\\s${IF_FLAG}\\s*=\\s*.+?[\\s|>]`)
const htmlparser = require('./htmlparser')
const { pluginOptions } = require('./resolve-options')

function getError (msg, node) {
  return new Error(
    `${chalk.bold.red('Vue template flags plugin error')}, ${chalk.red(msg)} at template:\n` +
    chalk.yellow(domserializer(node))
  )
}

// 0: blank text, 1: other, 2: if-flag, 3: elif-flag, 4: else-flag
function getState (node) {
  if (node.type === 'text' && !node.data.trim()) {
    return 0
  }
  if (node.type !== 'tag') { // not support 'script'
    return 1
  }
  if (IF_FLAG in node.attribs) {
    if (!node.attribs[IF_FLAG]) {
      throw getError(`Directive "${IF_FLAG}" can not be empty`, node)
    }
    return 2
  }
  if (ELIF_FLAG in node.attribs) {
    if (!node.attribs[ELIF_FLAG]) {
      throw getError(`Directive "${ELIF_FLAG}" can not be empty`, node)
    }
    return 3
  }
  if (ELSE_FLAG in node.attribs) {
    if (node.attribs[ELSE_FLAG]) {
      throw getError(`Directive "${ELSE_FLAG}" must be empty`, node)
    }
    return 4
  }
  return 1
}

function setFlagValue (node, attr, values, truthy) {
  try {
    const exp = (values || []).map(v => `!(${v})&&`).join('') + `(${truthy})`
    const keepElement = (new Function(`with(this){return ${exp}}`)).call(pluginOptions.flags) // eslint-disable-line
    node.__value = [attr, !!keepElement]
  } catch (e) {
    throw getError(`Unknown flag name, ${e.message}`, node)
  }
}

function getDirectives (dom) {
  let values = null; let prevState = 1
  for (let node of dom) {
    if (node.children) {
      getDirectives(node.children)
    }
    const currentState = getState(node)
    switch (currentState) {
      case 1: {
        values = null
        break
      }
      case 2: {
        const value = node.attribs[IF_FLAG]
        setFlagValue(node, IF_FLAG, null, value)
        values = [value]
        break
      }
      case 3: {
        if (prevState !== 2 && prevState !== 3) {
          throw getError(`Directive "${ELIF_FLAG}" must be next to "${IF_FLAG}"`, node)
        }
        const value = node.attribs[ELIF_FLAG]
        setFlagValue(node, ELIF_FLAG, [...values], value)
        values.push(value)
        break
      }
      case 4: {
        if (prevState !== 2 && prevState !== 3) {
          throw getError(`Directive "${ELSE_FLAG}" must be next to "${IF_FLAG}"`, node)
        }
        setFlagValue(node, ELSE_FLAG, [...values], 'true')
        values = null
        break
      }
      default: break
    }
    if (currentState !== 0) {
      prevState = currentState
    }
  }
}

function transformDom (dom) {
  getDirectives(dom)
  for (let node of dom) {
    if (node.__value) {
      const [attr, keepElement] = node.__value
      if (!keepElement) {
        if (node.parent) {
          const childs = node.parent.children
          childs.splice(childs.lastIndexOf(node), 1)
          node.parent = null
        }
      } else {
        delete node.attribs[attr]
        if (node.children) {
          transformDom(node.children)
        }
      }
    } else {
      if (node.children) {
        transformDom(node.children)
      }
    }
  }
}

module.exports = function transformTemplate (template, loaderContext) {
  if (!testDirective.test(template)) {
    return template
  }
  if (pluginOptions.watch && loaderContext) {
    loaderContext.addDependency(RESOLVED_FLAGS_PATH)
  }
  const ast = htmlparser(template.trim())
  if (ast.length !== 1 || ast[0].type !== 'tag') {
    return template
  }
  if ([IF_FLAG, ELIF_FLAG, ELSE_FLAG].filter(d => d in ast[0].attribs).length) {
    return template
  }

  try {
    transformDom(ast)
  } catch (e) {
    if (loaderContext) {
      loaderContext.emitError(e)
      return template
    }
    throw e
  }

  return domserializer({
    type: 'root',
    children: ast
  })
}
