...
function evaluateFlagDir (exps, flags) {
  const expression = exps.map((exp, i) => {
    if (i !== exps.length - 1) {
      return `!(${exp})`
    } else {
      return `(${exp})`
...
function transformNode (ast, flags, useComment) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    const keep = evaluateFlagDir(exps, flags)
    if (keep) {
...
module.exports = function (ast, pluginOptions, useComment) {
  if (!ast.parent) {
    ...
    if (checkDir && checkDir.length) {
      throw genFlagDirError(ast, `flag directives "${checkDir}" can not be used on root element`)
    }
