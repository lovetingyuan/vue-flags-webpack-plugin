...
module.exports.pitch = function pitch () {
  ...
  if (matched) {
    ...
    const flagName = JSON.stringify(matched.__flag_name__)
    const vueComponent = pluginOptions.watch && this.hot && /\.vue(\.html)?$/.test(this.resourcePath)
    const msg = `This module ${rawRequest} was ignored due to flag: ${flagName}${issuer ? ` at ${issuer}` : ''}.`
    const logMsg = JSON.stringify(`${PLUGIN_NAME} warning: ${msg}`)
